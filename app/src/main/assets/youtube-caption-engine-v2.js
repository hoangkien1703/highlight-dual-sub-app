(function (root, factory) {
      ...
      if (everyChunkAnchored) {
        for (let i = 0; i < tokenSegments.length; i += 1) {
          ...
          const usableEnd = Math.max(
            segmentStart + 0.04 * segment.tokens.length,
            segmentEnd
          );
          const slice = Math.max(
            0.04,
            (usableEnd - segmentStart) / segment.tokens.length
          );
        ...
        cueTokens.forEach((word, index) => {
          words.push({
            ...
            endSec: index === cueTokens.length - 1
              ? cueEnd
              : cueStart + slice * (index + 1),
  ...
  function findActiveCaption(cues, currentSecond, leadSeconds) {
    if (!Array.isArray(cues) || !cues.length || !Number.isFinite(currentSecond)) {
      return null;
    }
  ...
  function buildTimedTextUrls(baseUrl, pageOrigin) {
    ...
    if (currentOrigin && parsed.hostname.endsWith('youtube.com') &&
        currentOrigin.hostname.endsWith('youtube.com') &&
        parsed.origin !== currentOrigin.origin) {
  ...
  function reconcileDomState(previousState, text, currentSecond, secondsPerWord) {
    ...
    if (!previousState || !Array.isArray(previousState.tokens) ||
        !previousState.tokens.length) {
    ...
    if (activeToken) {
      ...
      if (mapped >= 0) {
        ...
        return {
          text: clean,
          tokens: currentTokens,
          activeWordIndex: mapped,
          anchorSec: second - mapped * step,
          lastSecond: second,
          lastTextChangeSec: second,
        };
      }
  ...
  function install(bridge) {
    ...
    function getPlayerResponse() {
      ...
      try {
        const configured = parsePlayerResponse(
    ...
    function visibleDomCaptionText() {
      ...
      if (segmentNodes.length) {
        return normalizeText(
          segmentNodes.map(node => node.textContent || '').join(' ')
        );
      }
    ...
    function ensureCaptionsEnabled() {
      const candidates = [
        ...
        ...Array.from(
          ...
          /caption|subtitle/i.test(node.getAttribute('aria-label') || '')
        ),
      ].filter(Boolean);
    ...
    function frameLoop() {
      ...
      const onFrame = function (_, metadata) {
        ...
        if (!emitTimedCaption(currentSecond)) {
          emitDomFallback(currentSecond);
        }
        frameLoop();
      };
      if (typeof video.requestVideoFrameCallback === 'function') {
        ...
        setTimeout(function () {
          onFrame(0, { mediaTime: video.currentTime });
        }, 40);
      }
    }
    ...
    const domObserver = new MutationObserver(function () {
      if (!cues.length) {
        emitDomFallback(Number.isFinite(lastMediaTime) ? lastMediaTime : 0);
      }
    });
    ...
    setInterval(function () {
      ...
      if (!cues.length && lastTimedTextFailureAt &&
          Date.now() - lastTimedTextFailureAt > 4000) {
        loadedVideoId = '';
